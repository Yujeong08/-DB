const CACHE='inventory-pwa-v4';
const ASSETS=['./','./index.html','./manifest.json','./db-bridge.js','./icon-192-v2.png','./icon-512-v2.png','./icon.svg'];
...
self.addEventListener('fetch',e=>{
  ...
  if(isDoc){
    e.respondWith(fetch(e.request).then(async r=>{
      ...
      const injected=t.includes('db-bridge.js')?t:t.replace('</head>','<script src="./db-bridge.js?v=1"></script></head>');
